...
export function usePrizeConfig() {
  ...
  const selectedGuaranteedPrize = ref<IPrizeConfig | null>(null);
  ...
  // 打开内定人员设置对话框
  function openGuaranteedDialog(prize: IPrizeConfig) {
    selectedGuaranteedPrize.value = prize;
  }

  // 更新内定人员列表
  function updateGuaranteedWinners(
    prizeId: number | string,
    guaranteedWinners: number[]
  ) {
    const prize = prizeList.value.find((p) => p.id === prizeId);
    if (prize) {
      prize.guaranteedWinners = guaranteedWinners;
      toast.success(i18n.global.t("error.success"));
    }
  }
