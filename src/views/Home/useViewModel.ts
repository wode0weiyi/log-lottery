...
export function useViewModel() {
  ...
  function initThreeJs() {
    ...
    const sphereVertices = createSphereVertices({
      objectsLength: objects.value.length,
    });
  ...
  function startLottery() {
    ...
    if (customCount && customCount.enable && customCount.countList.length > 0) {
      for (let i = 0; i < customCount.countList.length; i++) {
        if (
          ...
          leftover =
            customCount.countList[i].count -
            customCount.countList[i].isUsedCount;
          break;
        }
      }
    }
    luckyCount.value =
      leftover < luckyCount.value ? leftover : luckyCount.value;

    // 内定中奖逻辑：优先从内定名单中选取人员
    const guaranteedWinners = currentPrize.value.guaranteedWinners || [];
    const guaranteedCandidates: IPersonConfig[] = [];

    // 筛选出尚未中当前奖项的内定人员
    if (guaranteedWinners.length > 0) {
      for (const personId of guaranteedWinners) {
        const person = personPool.value.find((p) => p.id === personId);
        if (person) {
          guaranteedCandidates.push(person);
        }
      }
    }
    ...
    // 先选取内定人员
    luckyTargets.value = guaranteedCandidates.slice(0, guaranteedCount);

    // 如果内定人员不足，从普通人员池中补充
    const remainingCount = luckyCount.value - guaranteedCount;
    if (remainingCount > 0) {
      // 从人员池中移除已选中的内定人员
      const remainingPool = personPool.value.filter(
        (person) => !luckyTargets.value.some((lucky) => lucky.id === person.id)
      );
      const randomPicks = getRandomElements(remainingPool, remainingCount);
      luckyTargets.value = [...luckyTargets.value, ...randomPicks];
    }

    // 从人员池中移除所有已选中的人员
    luckyTargets.value.forEach((item) => {
      const index = personPool.value.findIndex(
  ...
  const init = () => {
    ...
    const checkAndInit = () => {
      ...
      if (
        allPersonList.value.length > 0 ||
        Date.now() - startTime >= maxWaitTime
      ) {
        console.log("初始化完成");
        tableData.value = initTableData({
          allPersonList: allPersonList.value,
          rowCount: rowCount.value,
        });
        initThreeJs();
        animation();
