...
export const useGlobalConfig = defineStore("global", {
  ...
  actions: {
    ...
    // 重置音乐列表
    resetMusicList() {
      this.globalConfig.musicList = JSON.parse(
        JSON.stringify(defaultMusicList)
      ) as IMusic[];
    },
    // 清空音乐列表
    clearMusicList() {
    ...
    addImage(image: IImage) {
      const hasSameId = this.globalConfig.imageList.some(
    ...
    updateImage(image: IImage) {
      const index = this.globalConfig.imageList.findIndex(
        (item) => item.id === image.id
      );
      if (index !== -1) {
        this.globalConfig.imageList[index] = image;
      }
    },
    // 重置图片列表
    resetImageList() {
